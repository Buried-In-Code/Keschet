...
object Keschet {
	...
	private val team1 = Team(Console.Colour.BLUE)
	private val team2 = Team(Console.Colour.RED)
	val board = Board()
		...
		do {
			...
			if (startLocation != null && board.getSquare(square = startLocation)!!.piece == null)
				...
				if (board.getSquare(square = startLocation)!!.piece!!.teamColour == team.colour) {
					...
					if (endLocation != null && board.getSquare(square = startLocation)!!.piece!!.validMovement(start = startLocation, end = endLocation)) {
	...
	private fun movePiece(start: Pair<Int, Int>, end: Pair<Int, Int>): Boolean {
		val direction = Board.calculateDirection(start, end)
		val distance = Math.abs(Board.calculateDistance(start, end))
	...
	private fun selectLocation(prompt: String): Pair<Int, Int>? {
		...
		do {
			try {
				...
				val row = Integer.valueOf(location[0])
				val column = Integer.valueOf(location[1])
